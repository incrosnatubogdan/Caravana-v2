...
class Kohana_Pagination
{
	...
	public function url($page = 1)
	{
		...
		{
			case 'replace':
				return URL::site(str_replace($this->config['current_page']['key'], $page, $this->config['current_page']['url']));

			case 'query_string':
				return URL::site(Request::current()
				                        ->uri()) . URL::query(array ($this->config['current_page']['key'] => $page));

			case 'route':
				return URL::site(Request::current()->route()
