...
(function($){
	...
	var Redactor = function(element, options)
	{
		...
		//if(options.adminAuthentication)
		//{
			var modal_image_check = String() + 
				'<div id="redactor_tabs" style="display: none;">' +
				...
				*/
					'<div id="redactor_tab2" class="redactor_tab" style="display: none;">' +
						'<div id="redactor_image_box_loading">PLEASE WAIT.... LOADING ....</div>' +
						'<div id="redactor_image_box" style="display:none;"></div>' +
						'<input type="hidden" id="redactor_image_box_choosen_file" name="redactor_image_box_choosen_file"/>' +
					'</div>' +
				/*
					'<div id="redactor_tab4" class="redactor_tab" style="display: none;">' +
						'<iframe style="width:570px;height:420px;border:0;margin-left:-30px;padding:0;" src="'+url_base+'media/js/admin/elfinder/elfinder.html?v1"></iframe>' +
					'</div>'+
				*/
				'</form>';// +
				/*
		...
		this.opts = $.extend({
			...
			imageGetJson: true,//'/folder/images.json',// '/media/js/admin/elfinder/php/../files/',// ,//false, //or 
	...
	Redactor.prototype = {
		...
		init: function()
		{
			if(!this.opts.adminAuthentication)
			{
				this.opts.fileUpload = false; // url
				this.opts.fileUploadCallback = false; // function			
			}
			// get path to styles
			this.getPath();
		...
		build: function()
		{
			...
			// frame
			this.$frame = $('<iframe id="elfinder_redactor_frame" frameborder="0" scrolling="auto" style="height: ' + this.height + ';" class="redactor_frame"></iframe>');
		...
		{	var css = '';
			//console.log('box:' + this.opts.box);
			//console.log(this.opts.css);
			if(typeof(this.opts.css) != 'object'){
				css = '<link media="all" type="text/css" href="' + this.opts.path + '/css/' + this.opts.css + '" rel="stylesheet">';	
			}else{
				$.each(this.opts.css, function(key, value) { 				  
				  css+='<link media="all" type="text/css" href="' + value + '" rel="stylesheet">';		
				});	
			}
			
			var frameHtml = '<!DOCTYPE html>\n' +
			'<html><head>' + css + '</head>' +
			'<body><div id="page" class="' + ((this.opts.box!=false)?this.opts.box:'') + '" contenteditable="true" dir="' + this.opts.direction + '">' +
			html + 
			'</div></body></html>';
		...
		resizeImage: function(resize)
		{
			...
			{
				$(resize).attr('_moz_resizing', true);
			});
			
			$(resize).dblclick($.proxy(function(e) //click
			{
				if (clicker)
				{
					this.imageEdit(e);
		 ...
		 * My:
		 */		
		imageLoadingModal: function(){
			//open modal
			$('#redactor_image_box_loading').show();
			$('#redactor_image_box').hide();
			var temp_container = this;
			if(!$('#elfinder').hasClass('ui-helper-reset')){
				setTimeout(function(){
					temp_container.imageLoadingModal();
				}, 1000);
			} else {
				//close modal
				$('#redactor_image_box_loading').hide();
				$('#redactor_image_box').show();
				$('#elfinder').height($('#redactor_image_box').height() - 20);
				//$('.elfinder-cwd-icon-image').each(function(){});
			}
		},
		 ...
		 * My:
		 */
		checkIfImageByHiddenInput: function(){
			var temp_container = this;
			if($('#redactor_image_box_choosen_file').val() == ''){
				setTimeout(function(){
					temp_container.checkIfImageByHiddenInput();
				}, 1000);
			} else {
				if(typeof($('#redactor_image_box_choosen_file').val()) != 'undefined'){
					var url = $('#redactor_image_box_choosen_file').val();
					$('#redactor_image_box_choosen_file').val('');
					temp_container.imageSetThumbByUrl(url);
				}
			}
		},
		showImage: function()
		{
			...
			var handler = $.proxy(function()
			{
				...
				if (this.opts.imageGetJson !== false)
				{
					...
					var temp_container = this;
				     $.ajax({
				        method: "post",
				        url: url_base+'administration/text_box/elfinder',
				        id: '',
				        data: '',
				        success: function(html) {
				            if (html != '') {
								$('#redactor_image_box').html(html);
								temp_container.imageLoadingModal();
								temp_container.checkIfImageByHiddenInput();
							}
				        }
			...
			this.modalInit(RLANG.image, 'image', 800, handler, endCallback, true);
		...
		imageSetThumbByUrl: function(url)
		{
			this._imageSet('<img alt="" src="' + url + '" />', true);
		},
		...
		modalInit: function(title, url, width, handler, endCallback, scroll)
		{
			...
			if (this.isMobile() === false)
			{
				$('#redactor_modal').css({ position: 'fixed', top: '50%', left: '50%', width: width + 'px', height: 'auto', marginTop: '-350px', marginLeft: '-' + (width+60)/2 + 'px' }).fadeIn('fast'); //' + (height+10)/2 + '
			}
			else
			{
			...
			if (scroll === true)
			{
				$('#redactor_image_box').height(500).width($('#redactor_modal').width() - 55).css('overflow', 'auto');
			}
